...
exports.loadVWAPDeltaGammaConfig =
	exports.createVWAPDeltaGammaCache =
	exports.VWAPDeltaGammaStrategy =
		void 0;
...
const strategyLogger = (0, logger_1.createLogger)("vwap-delta-gamma");
...
class VWAPDeltaGammaStrategy {
	...
	buildVwapContext(executionCandles, trendCandles) {
		...
		const daily = this.buildVwapDelta(
			...
			price,
			prevPrice,
			indicators_1.calculateDailyVWAP
		);
		const weekly = this.buildVwapDelta(
			...
			price,
			prevPrice,
			indicators_1.calculateWeeklyVWAP
		);
		const monthly = this.buildVwapDelta(
			...
			price,
			prevPrice,
			indicators_1.calculateMonthlyVWAP
		);
		const rolling50 = this.buildVwapDelta(
			...
			prevPrice,
			(candles) =>
				(0, indicators_1.calculateRollingVWAP)(
					candles,
	...
	buildAtrContext(executionCandles, trendCandles) {
		...
		const atrSeries = (0, indicators_1.calculateATRSeries)(
			atrInputs,
		...
		const atr5m = (0, indicators_1.calculateAtr5m)(
			atr5mInputs,
...
const createVWAPDeltaGammaCache = (
	clientFetcher,
	symbol,
	timeframes,
	maxAgeMs
) =>
	(0, multiTimeframeCache_1.createMTFCache)({
		symbol,
		timeframes,
		maxAgeMs,
...
const loadVWAPDeltaGammaConfig = (
	configPath = node_path_1.default.join(
		(0, config_1.getWorkspaceRoot)(),
		"configs",
		"strategies",
