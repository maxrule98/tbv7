...
exports.loadVWAPDeltaGammaConfig = exports.createVWAPDeltaGammaCache = exports.VWAPDeltaGammaStrategy = void 0;
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const models_quant_1 = require("@agenai/models-quant");
const indicators_1 = require("@agenai/indicators");
const multiTimeframeCache_1 = require("../../data/multiTimeframeCache");
const DEFAULT_MACD_FAST = 12;
const DEFAULT_MACD_SLOW = 26;
const DEFAULT_MACD_SIGNAL = 9;
const DEFAULT_MACD_WINDOW = 20;
class VWAPDeltaGammaStrategy {
    constructor(config, deps) {
        this.config = config;
        this.deps = deps;
    }
    async decide(position = "FLAT") {
        const executionCandles = await this.deps.cache.getCandles(this.config.timeframes.execution);
        if (executionCandles.length < this.config.vwapRollingLong + 5) {
            return this.noAction(executionCandles, "insufficient_candles");
        }
        const trendCandles = await this.deps.cache.getCandles(this.config.timeframes.trend);
        const biasCandles = await this.deps.cache.getCandles(this.config.timeframes.bias);
        const macroCandles = await this.deps.cache.getCandles(this.config.timeframes.macro);
        ...
        const mtfBias = this.computeBiasSummary(trendCandles, biasCandles, macroCandles);
        const macdForecast = this.computeMacdForecast(executionCandles);
        const deltaHistory = this.computeDeltaHistory(executionCandles, vwapContext.daily.value);
        const trendFlags = this.evaluateSetups(latest, prev, vwapContext, atrContext, mtfBias, macdForecast, deltaHistory);
        const recommendations = this.computeRecommendations(latest, vwapContext, atrContext);
        ...
        if (position === "LONG" && trendFlags.longExitReason) {
            return this.tradeIntent(latest, "CLOSE_LONG", trendFlags.longExitReason, recommendations);
        }
        if (position === "SHORT" && trendFlags.shortExitReason) {
            return this.tradeIntent(latest, "CLOSE_SHORT", trendFlags.shortExitReason, recommendations);
        }
        if (position === "FLAT") {
            if (trendFlags.trendLong || trendFlags.meanRevLong || trendFlags.breakoutLong) {
            ...
            if (trendFlags.trendShort || trendFlags.meanRevShort || trendFlags.breakoutShort) {
    ...
    buildVwapContext(executionCandles, trendCandles) {
        ...
        const prevPrice = prevExecution.length
            ...
            : null;
        const daily = this.buildVwapDelta(executionCandles, prevExecution, price, prevPrice, indicators_1.calculateDailyVWAP);
        const weekly = this.buildVwapDelta(executionCandles, prevExecution, price, prevPrice, indicators_1.calculateWeeklyVWAP);
        const monthly = this.buildVwapDelta(executionCandles, prevExecution, price, prevPrice, indicators_1.calculateMonthlyVWAP);
        const rolling50 = this.buildVwapDelta(executionCandles, prevExecution, price, prevPrice, (candles) => (0, indicators_1.calculateRollingVWAP)(candles, this.config.vwapRollingShort));
        const rolling200 = this.buildVwapDelta(executionCandles, prevExecution, price, prevPrice, (candles) => (0, indicators_1.calculateRollingVWAP)(candles, this.config.vwapRollingLong));
        ...
        const trendPrevPrice = trendPrev.length
            ...
            : null;
        const trendRolling50 = trendPrice === null
            ? null
            : this.buildVwapDelta(trendCandles, trendPrev, trendPrice, trendPrevPrice, (candles) => {
                const period = Math.min(this.config.vwapRollingShort, candles.length);
                if (period <= 0) {
                    return null;
                }
                return (0, indicators_1.calculateRollingVWAP)(candles, period);
            });
        return {
            daily,
            weekly,
    ...
    buildVwapDelta(candles, previousCandles, price, prevPrice, calculator) {
        const value = calculator(candles);
        const previous = calculator(previousCandles);
        const prevDelta = prevPrice !== null && previous !== null ? prevPrice - previous : null;
        return {
            value,
            previous,
            delta: (0, indicators_1.computeDeltaGamma)(price, value, prevDelta),
        };
    }
    buildAtrContext(executionCandles, trendCandles) {
        const atrInputs = executionCandles.map((candle) => ({
            high: candle.high,
            low: candle.low,
            close: candle.close,
        }));
        const atrSeries = (0, indicators_1.calculateATRSeries)(atrInputs, this.config.atrPeriod1m);
        ...
        const atr5m = (0, indicators_1.calculateAtr5m)(atr5mInputs, this.config.atrPeriod5m);
        const rising = atrLen >= 3 &&
            atrSeries[atrLen - 3] < atrSeries[atrLen - 2] &&
            atrSeries[atrLen - 2] < atrSeries[atrLen - 1];
        const low = atr1m !== null &&
            atr1mAvg !== null &&
            atr1mAvg > 0 &&
            atr1m / atr1mAvg <= this.config.atrLowThreshold;
        const expanding = atr1m !== null &&
            atr1mPrev !== null &&
            atr1mPrev > 0 &&
    ...
    computeBiasSummary(trendCandles, biasCandles, macroCandles) {
    ...
    computeMacdForecast(candles) {
        ...
        return (0, models_quant_1.ar4Forecast)(window);
    }
    buildHistogramSeries(closes) {
        if (closes.length === 0) {
            return [];
        }
        ...
        const signalSeries = this.calculateSignalSeries(macdSeries, DEFAULT_MACD_SIGNAL);
    ...
    calculateSignalSeries(values, length) {
        ...
        const multiplier = 2 / (length + 1);
        let signal = values.find((value) => value !== null) ?? null;
    ...
    evaluateSetups(latest, prev, vwap, atr, bias, macdForecast, deltaHistory) {
        ...
        const macdUpward = macdForecast !== null &&
            macdForecast > this.config.macdForecastDeadband;
        const macdDownward = macdForecast !== null &&
            macdForecast < -this.config.macdForecastDeadband;
        const fiveMinuteDeltaPositive = vwap.trendRolling50?.delta?.deltaSign === "positive";
        const fiveMinuteDeltaNegative = vwap.trendRolling50?.delta?.deltaSign === "negative";
        const trendLong = priceAboveAll &&
            delta200Positive &&
            gamma200Positive &&
            pullbackTouch &&
            ...
            bias.trend === "bull" &&
            bias.macro !== "bear";
        const trendShort = priceBelowAll &&
        ...
        const priceBelowDailyExtreme = vwap.daily.value !== null &&
            atrReference > 0 &&
            price < vwap.daily.value - 2 * atrReference;
        const priceAboveDailyExtreme = vwap.daily.value !== null &&
            atrReference > 0 &&
            price > vwap.daily.value + 2 * atrReference;
        const deltaDailyExtremeNegative = vwap.daily.delta.delta !== null &&
            atrReference > 0 &&
            vwap.daily.delta.delta <=
                -this.config.deltaExtremeMultiplier * atrReference;
        const deltaDailyExtremePositive = vwap.daily.delta.delta !== null &&
            atrReference > 0 &&
            vwap.daily.delta.delta >=
                this.config.deltaExtremeMultiplier * atrReference;
        const gammaDailyPositiveFlip = vwap.daily.delta.gammaSign === "positive" &&
            vwap.daily.delta.gammaFlipped;
        const gammaDailyNegativeFlip = vwap.daily.delta.gammaSign === "negative" &&
            vwap.daily.delta.gammaFlipped;
        const meanRevLong = priceBelowDailyExtreme &&
            deltaDailyExtremeNegative &&
            gammaDailyPositiveFlip &&
            macdUpward;
        const meanRevShort = priceAboveDailyExtreme &&
            deltaDailyExtremePositive &&
            gammaDailyNegativeFlip &&
            macdDownward;
        const betweenDailyWeekly = this.isBetween(price, vwap.daily.value, vwap.weekly.value);
        const prevBelowDaily = prevClose !== null &&
            vwap.daily.value !== null &&
            prevClose <= vwap.daily.value;
        const prevAboveDaily = prevClose !== null &&
            vwap.daily.value !== null &&
            prevClose >= vwap.daily.value;
        const deltaSmall = vwap.daily.delta.deltaMagnitude !== null &&
            vwap.daily.delta.deltaMagnitude <= this.config.minPullbackDistance;
        const gammaNearZero = vwap.daily.delta.gammaMagnitude !== null &&
            vwap.daily.delta.gammaMagnitude <= this.config.minPullbackDistance;
        const breakoutLong = betweenDailyWeekly &&
            atr.low &&
            deltaSmall &&
            gammaNearZero &&
            prevBelowDaily &&
            ...
            atr.expanding;
        const breakoutShort = betweenDailyWeekly &&
            atr.low &&
            deltaSmall &&
            gammaNearZero &&
            prevAboveDaily &&
        ...
        const longExitReason = this.pickLongExitReason(vwap, atr, price, deltaWeakening);
        const shortExitReason = this.pickShortExitReason(vwap, atr, price, deltaStrengthening);
        const trendRegime = priceAboveAll
            ? "bull_trend"
            : priceBelowAll
                ? "bear_trend"
                : "range";
        const volatilityRegime = atr.expanding
            ? "expansion"
            : atr.low
                ? "compression"
                : "balanced";
    ...
    computeDeltaHistory(candles, reference, lookback = 3) {
        if (reference === null) {
            return [];
        }
    ...
    pickLongExitReason(vwap, atr, price, deltaWeakening) {
        if (vwap.rolling50.value !== null &&
            price < vwap.rolling50.value) {
            return "trend_vwap50_break";
        }
        if (vwap.rolling50.delta.gammaSign === "negative" && vwap.rolling50.delta.gammaFlipped) {
            return "trend_gamma50_flip_down";
        }
        if (deltaWeakening) {
            return "trend_daily_delta_weakening";
        }
        if (vwap.daily.value !== null &&
    ...
    pickShortExitReason(vwap, atr, price, deltaStrengthening) {
        if (vwap.rolling50.value !== null &&
            price > vwap.rolling50.value) {
            return "trend_vwap50_break";
        }
        if (vwap.rolling50.delta.gammaSign === "positive" && vwap.rolling50.delta.gammaFlipped) {
    ...
    computeRecommendations(latest, vwap, atr) {
        ...
        if (vwap.daily.value !== null && vwap.weekly.value !== null && atrValue > 0) {
            const clusterLow = Math.min(vwap.daily.value, vwap.weekly.value, vwap.monthly.value ?? vwap.daily.value);
            const clusterHigh = Math.max(vwap.daily.value, vwap.weekly.value, vwap.monthly.value ?? vwap.weekly.value);
            levels.breakoutLong = {
    ...
    isAbove(price, reference) {
        return reference !== null ? price > reference : false;
    }
    isBelow(price, reference) {
        return reference !== null ? price < reference : false;
    }
    isBetween(price, first, second) {
    ...
    tradeIntent(latest, intent, reason, recommendations) {
        return {
...
exports.VWAPDeltaGammaStrategy = VWAPDeltaGammaStrategy;
const createVWAPDeltaGammaCache = (clientFetcher, symbol, timeframes, maxAgeMs) => (0, multiTimeframeCache_1.createMTFCache)({
    symbol,
    timeframes,
    maxAgeMs,
    fetcher: clientFetcher,
});
exports.createVWAPDeltaGammaCache = createVWAPDeltaGammaCache;
const loadVWAPDeltaGammaConfig = (configPath = node_path_1.default.join(process.cwd(), "configs", "strategies", "vwap-delta-gamma.json")) => {
    const contents = node_fs_1.default.readFileSync(configPath, "utf-8");
    return JSON.parse(contents);
};
exports.loadVWAPDeltaGammaConfig = loadVWAPDeltaGammaConfig;
