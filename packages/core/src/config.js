...
exports.loadAgenaiConfig =
	exports.loadAccountConfig =
	exports.loadRiskConfig =
	exports.loadStrategyConfig =
	exports.loadExchangeConfig =
...
const findWorkspaceRoot = () => {
	...
	while (
		!WORKSPACE_SENTINELS.some((file) =>
			node_fs_1.default.existsSync(node_path_1.default.join(current, file))
		)
	) {
...
const getWorkspaceRoot = () => findWorkspaceRoot();
exports.getWorkspaceRoot = getWorkspaceRoot;
const getDefaultEnvPath = () =>
	node_path_1.default.join(findWorkspaceRoot(), ".env");
const getDefaultConfigDir = () =>
	node_path_1.default.join(findWorkspaceRoot(), "config");
...
const loadExchangeConfig = (
	env,
	configDir = getDefaultConfigDir(),
	exchangeProfile
) => {
	const profile = exchangeProfile ?? env.exchangeId ?? "mexc";
	const exchangePath = node_path_1.default.join(
		configDir,
		"exchange",
		`${profile}.json`
	);
...
const loadStrategyConfig = (
	configDir = getDefaultConfigDir(),
	strategyProfile = "macd_ar4"
) => {
	const strategyPath = node_path_1.default.join(
		configDir,
		"strategies",
...
const loadRiskConfig = (
	...
	const riskPath = node_path_1.default.join(
		configDir,
		"risk",
		`${riskProfile}.json`
	);
...
const loadAccountConfig = (
	...
	const accountPath = node_path_1.default.join(
		configDir,
		"account",
		`${accountProfile}.json`
	);
...
const loadAgenaiConfig = (options = {}) => {
	...
	const envPath =
		options.envPath ?? node_path_1.default.join(workspaceRoot, ".env");
	const configDir =
		options.configDir ?? node_path_1.default.join(workspaceRoot, "config");
	const env = (0, exports.loadEnvConfig)(envPath);
	return {
		env,
		exchange: (0, exports.loadExchangeConfig)(
			env,
			configDir,
			options.exchangeProfile
		),
		strategy: (0, exports.loadStrategyConfig)(
			configDir,
